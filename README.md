# HOOK
Đây là một trò chơi câu cá đơn giản.
## Cách chơi :
Tải file main.exe về và chơi.
## V1 :
Các chức năng cơ bản:
- Câu cá.
- Xem thông tin người chơi .
- Xem kho cá.
## Update V1.1 :
Thêm chức năng:
- Bán cá.
- Tỉ lệ câu cá theo độ hiếm.

Thêm hash table và fish database.
## Update V1.2:
Thêm rod database, sửa lại cấu trúc của fish database.
Thêm chức năng:
- Mua cần câu ( mỗi cần câu có một buff tỉ lệ khác nhau ( GPT hỗ trợ ) ).

Sửa lại cấu trúc game.
## Update V1.3:
Thêm chức năng:
- Sắp xếp kho cá theo 3 tiêu chí tên, khối lượng, độ hiếm.
- Tạo thêm thread hỗ trợ treo máy, tự động nhận lệnh từ bàn phím để thoát chế độ auto. ( GPT )

Thời gian câu cá phụ thuộc vào độ hiếm của cá
## Update V1.4:
Thêm chức năng:
- Đăng nhập.
- Bảng xếp hạng theo level và coin. ( GPT hỗ trợ )

Thay đổi cách lưu trữ inventory.
## Update V1.5:
Thêm mã hóa dữ liệu save, đảm bảo người chơi khó khăn trong việc thay đổi dữ liệu, tránh việc gian lận. Chỉnh sửa cách lưu và trích xuất dữ liệu.